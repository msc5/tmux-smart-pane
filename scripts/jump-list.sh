...
#
# Usage:
#   jump-list.sh sessions       — local + remote sessions, globally sorted by @last_seen
#   jump-list.sh sessions-fast  — local sessions only, no SSH (for instant fzf load)
#   jump-list.sh panes          — one row per pane, sorted by @last_seen recency
PLUGIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$PLUGIN_DIR/scripts/helpers.sh"

# Unsorted rows (sort-key prefix retained) for local sessions.
_jump_list_local_rows() {
    local now="$1"
...
# Unsorted rows (sort-key prefix retained) for remote sessions via parallel SSH.
_jump_list_remote_rows() {
    local now="$1"
...
_jump_list_sessions_fast() {
    local now; now="$(date +%s)"
    _jump_list_local_rows "$now" | sort -n -t'|' -k1,1 | cut -d'|' -f2-
}

_jump_list_sessions() {
    local now; now="$(date +%s)"
    { _jump_list_local_rows "$now"; _jump_list_remote_rows "$now"; } \
...
case "${1:-sessions}" in
    ...
    sessions-fast) _jump_list_sessions_fast ;;
